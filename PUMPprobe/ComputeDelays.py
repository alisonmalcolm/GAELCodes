...
# read in data from the scope and make a matrix of values from it
    ...
    for line in f:
        ...
        if nch==2:
            tmp2.append(float(columns[1]))
            tmp3.append(float(columns[2]))

    dataset.time=np.asarray(tmp1)
    if nch==1:
        dataset.data=np.asarray(tmp2)
    if nch==2:
        dataset.data=np.zeros([np.size(tmp1),2])
        dataset.data[:,0]=np.asarray(tmp2)
        dataset.data[:,1]=np.asarray(tmp3)
    f.close()
...
#Compute the delay between the perturbed and un-perturbed pump signals
    ch_ind=0

    #first check (and fix) time axes shifts
    dt1=PUMPprobe[ch_ind].time[1]-PUMPprobe[ch_ind].time[0]
    dt2=PUMP[ch_ind].time[1]-PUMP[ch_ind].time[0]
    dt3=probe[ch_ind].time[1]-probe[ch_ind].time[0]

    #difference in dts should be less than a ns

    if np.max([np.abs(dt1-dt2),np.abs(dt2-dt3),np.abs(dt1-dt3)]) > 1.e-9:
        print("dts are not close enought, returning 0")
        return 0.0

    #take all of the data beginning from the latest start time to the earliest end time
    t0=np.max([PUMPprobe[ch_ind].time[0],PUMP[ch_ind].time[0],probe[ch_ind].time[0]])
    t_end=np.min([PUMPprobe[ch_ind].time[-1],PUMP[ch_ind].time[-1],probe[ch_ind].time[-1]])
    
    it0_Pp=np.argmin(np.abs(PUMPprobe[ch_ind].time-t0))
    it0_PU=np.argmin(np.abs(PUMP[ch_ind].time-t0))
    it0_Pr=np.argmin(np.abs(probe[ch_ind].time-t0))

    ite_Pp=np.argmin(np.abs(PUMPprobe[ch_ind].time-t_end))
    ite_PU=np.argmin(np.abs(PUMP[ch_ind].time-t_end))
    ite_Pr=np.argmin(np.abs(probe[ch_ind].time-t_end))

    PUMPprobe_init=PUMPprobe[ch_ind].data[it0_Pp:ite_Pp]
    PUMP_init=PUMP[ch_ind].data[it0_PU:ite_PU]
    probe_init=probe[ch_ind].data[it0_Pr:ite_Pr]

    t_complete=np.arange(t0,t_end,dt1)

    #first subtract the pump signal from the perturbed probe signal
#    size_PUMPprobe=np.shape(PUMPprobe_init)[0]
#    size_PUMP=np.shape(PUMP_init)[0]
    
#    if size_PUMPprobe != size_PUMP:
#        if size_PUMPprobe < size_PUMP:
#            PUMPprobe_init=np.delete(PUMPprobe_init,range(size_PUMPprobe-1,size_PUMP-1))
#        else:
#            PUMPprobe_init=np.delete(PUMPprobe_init,range(size_PUMP-1,size_PUMPprobe-1))
        
    #Perturbed_probe=np.subtract(PUMP_only[ch_ind].data,PUMP_probe[ch_ind].data)
    Perturbed_probe=np.subtract(PUMPprobe_init,PUMP_init)
    #Perturbed_probe=PUMP_only[ch_ind].data

#    size_Perturbed_probe=np.size(Perturbed_probe)
#    size_probe=np.size(probe_init)

#    if size_Perturbed_probe != size_probe:
#        if size_Perturbed_probe < size_probe:
#            Perturbed_probe=np.delete(Perturbed_probe,range(size_Perturbeprobe-1,size_probe-1))
#        else:
#            Perturbed_probe=np.delete(Perturbed_probe,range(size_probe-1,size_Perturbed_probe-1))
...
#    probe_only_0mean=probe_only[ch_ind].data-np.mean(probe_only[ch_ind].data)
    ...
    if probe_filter_cut > fs:
        probe_filter_cut=0.9*fs


    filter_order=5    

    Perturbed_probe_filtered=butter_lowpass_filter(Perturbed_probe,probe_filter_cut,fs,filter_order)
    probe_filtered=butter_lowpass_filter(probe_init,probe_filter_cut,fs,filter_order)
...
#    probe_filtered=probe[ch_ind].data
    ...
    index_of_tmin=np.argmin(np.abs(tmin_to_correlate-t_complete))
    index_of_tmax=np.argmin(np.abs(tmax_to_correlate-t_complete))
    ...
    probe_to_correlate=probe_filtered[tvec]*tap
    Perturbed_probe_to_correlate=Perturbed_probe_filtered[tvec]*tap
#    probe_to_correlate=probe_filtered[tvec]/np.max(np.abs(probe_filtered[tvec]))*tap
#    Perturbed_probe_to_correlate=Perturbed_probe_filtered[tvec]/np.max(np.abs(Perturbed_probe_filtered[tvec]))*tap


#    plt.plot(probe_to_correlate)
#    plt.plot(Perturbed_probe_to_correlate)
#    plt.xlim([500,1500])
#    plt.show()

#    print np.shape(probe_filtered)
#    print np.shape(Perturbed_probe_filtered)
#    print np.min(tvec)
#    print np.max(tvec)
#    print np.size(t_complete)

    Correlation_of_probes=np.correlate(probe_to_correlate,Perturbed_probe_to_correlate,"full")
...
#    print(delayvec)
    if np.size(delayvec)>2:
        delaystep2=delayvec[2]-delayvec[1]
        if np.abs(delaystep2-delaystep) > .1:
            print("delaystep seems wrong, fixing")
            delaystep=delaystep2
...
#    print delayvec
    ...
    for delay in delayvec:
        ...
        tshift[j]=Compute_single_delay(probe_only0,probe_only,PUMP0,1,winlen)
...
def Compute_Raw_Delays(delayvec,delay_scale_fact,missing_data,delaystep,path,fname_part2,fnameend,winlen,recompute,flipdelays=0,outlier_thresh=1000.0):
...
#    tshifts.remove_noise=butter_lowpass_filter(tshifts.raw,150e3,fs,2)
    ...
    tshifts.remove_noise_wiggles=butter_lowpass_filter(tshifts.raw,50e3,fs,2)
    tshifts.delays=delayvec
...
#def cosinetaper(numpts,tapperc):
#hanning window taper on both sides of input data
#numpts is the number of points in the output taper
#tapperc is the percentage of the signal to taper in total 
#on both sides

    if tapperc > 100:
        print("tapperc is in percent, should be < 100")
...
def GeneratePathlist(rootdir,excludelist,pathlist,includelist=[]):
    ...
    if len(subdirlist)==0:
        print('rootdir')
        print(rootdir)
        #but first check that the data are there
        cond1=os.path.isfile(rootdir+'/STrans_B4.csv')
        filelist=glob.glob(rootdir+"/pp30.25.csv")
        cond2=(len(filelist) is not 0)
        cond3=True
        for text in includelist:
            if text not in rootdir:
                cond3=False

        print('conditions are',cond1,cond2,cond3)
...
#        labeltxt=pathend#pathsplit[len(pathsplit)-2]
        labeltxt=pathsplit[len(pathsplit)-2][5:-5]
        linesym='-'
...
def GetDelays(pathlist,maxvalpump,winlen=60e-6,savefig=0,pathbase='',compbase='',recompute=0,missing_data=np.array([]),delaystep=1.0,delay_scale_fact=1,doslopes=0,plotcolor='O',outlier_thresh=1000.):
#This computes the delays and plots: (i) the delays for all of the data (ii) the delays as a function of input voltage (ii) the wiggle amplitude (which is actually the amplitude of a best-fit sinusoid) (iii) the delays as a function of the maximum recorded pump amplitude and (iv) the wiggle amplitude as a function of the maximum recorded pump amplitude.  
